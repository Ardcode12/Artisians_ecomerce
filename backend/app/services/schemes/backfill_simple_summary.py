...
def backfill_existing_schemes():
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, COALESCE(scheme_name, name) as scheme_name, COALESCE(provider_name, ministry) as provider_name,
                   COALESCE(scheme_category, category) as scheme_category,
                   COALESCE(eligibility_summary, eligibility_json) as eligibility_summary,
                   COALESCE(benefits_offered, benefits_json) as benefits_offered,
