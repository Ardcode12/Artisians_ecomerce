...
def init_db():
    ...
    with get_db() as conn:
        ...
        if is_postgres():
            cursor.execute("ALTER TABLE products ADD COLUMN IF NOT EXISTS target_units INTEGER DEFAULT 0;")
        else:
            try:
...
def _seed_design_ideas():
    ...
    with get_db() as conn:
        ...
        count = _row_val(cursor.fetchone())
        if count == 0:
...
def _seed_demand_forecasts():
    ...
    with get_db() as conn:
        ...
        festivals = [
            {
                ...
                "quote_rationale": "Festival demand starts 25 days before Diwali. Artisans who set production goals early achieve 3.2x higher sell-through.",
                "tip": "Procure raw materials by Oct 1. Complete batch production by Oct 12 to guarantee 100% on-time festive dispatches.",
                "prep_lead_days": 30
            },
            {
                "slug": "pongal",
        ...
        forecasts = [
            {
                ...
                "uplift_pct": 45,
                "recommended_qty_min": 50,
                "recommended_qty_max": 80,
                "target_date": "Oct 12",
            ...
            },
            {
                ...
                "expected_demand": "High",
                "uplift_pct": 40,
                "recommended_qty_min": 35,
                "recommended_qty_max": 50,
                "target_date": "Oct 10",
                "confidence": "Estimated — based on category trends and buyer pre-order signals",
        ...
        sup_count = _row_val(cursor.fetchone())
        if sup_count == 0:
            now_iso = datetime.now(timezone.utc).isoformat()
            import uuid
            seeded_suppliers = [
        ...
        act_count = _row_val(cursor.fetchone())
        if act_count == 0:
            now_iso = datetime.now(timezone.utc).isoformat()
            seeded_activities = [
                {
